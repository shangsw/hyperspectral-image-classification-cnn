...
data_filename = ['PaviaU_train_1d.mat',
                 'PaviaU_train_2d.mat']
...
count = 500 #the  training samples
...
train_patch_1d_noise = input_train_patch_1d + 1e-6*np.random.normal(0.0, 1.0, input_train_patch_1d.shape)
...
#save 1d-data
train_1d = {}
file_name = 'PaviaU_train_1d_wa.mat' 
...
'''
#copy data without augment  
for i in range(class_num):
    while len(train_patch_2d[i]) < count:
        j = randint(0,len(train_patch_2d[i])-1)
        train_patch_2d[i] = np.vstack((train_patch_2d[i], train_patch_2d[i][j][np.newaxis, :]))
...
#save 2d data
train_2d = {}
file_name = 'PaviaU_train_2d_wa.mat'
