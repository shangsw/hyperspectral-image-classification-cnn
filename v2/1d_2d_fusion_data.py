...
with tf.Session() as sess:
    saver = tf.train.import_meta_graph('./model/1dcnn_model/1dcnn-model-10000.meta')
    saver.restore(sess, './model/1dcnn_model/1dcnn-model-10000')
    ...
    for i in range(int(input_labels.shape[0]/batch_size)+1):
        start = i * batch_size
        end = start + batch_size
        if end > input_labels.shape[0]:
            end = input_labels.shape[0]
        train_feature_1d.extend(h_feature_1d.eval(feed_dict={
            x_1d: input_data_1d[start:end], y_1d: input_labels[start:end], keep_prob: 1.0}))
...
tf.reset_default_graph()
with tf.Session() as sess:
    saver = tf.train.import_meta_graph('./model/2dcnn_model/2dcnn-model-10000.meta')
    ...
    for i in range(int(input_labels.shape[0]/batch_size)+1):
        ...
        train_feature_2d.extend(h_feature_2d.eval(feed_dict={
            x_2d: input_data_2d[start:end], y_2d: input_labels[start:end], keep_prob: 1.0}))
    #get test data's 1d-ne feature
    for i in range(int(eval_labels.shape[0]/batch_size_for_test)+1):
        start = i * batch_size_for_test 
        end = start + batch_size_for_test
...
train_dict["train_labels"] = input_labels
...
input_1d['train_labels'] = input_labels
